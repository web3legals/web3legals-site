...
Calls Groq API directly (no gateway needed for Groq)
Uses Cloudflare AI Gateway only for Gemini fallback
D1 via Worker bridge with fixed auth
"""
...
def worker_call(method, path, body=None):
    """Call the Cloudflare Worker D1 bridge."""
    if not CF_WORKER_URL or not CF_WORKER_SECRET:
        print(f"  Worker not configured — skipping {path}")
        return None
    ...
    try:
        ...
        print(f"  Worker HTTP {e.code} on [{method} {path}]: {body_text[:200]}")
        return None
...
def d1_check_seen(aids):
    ...
    print("  D1 unavailable — treating all as unseen")
...
def d1_mark_seen(aids):
    ...
    if not result:
        ...
        print(f"  D1 unavailable — saved {len(aids)} to local .seen_articles.json")

def d1_upsert_articles(articles):
    result = worker_call("POST", "/articles/upsert", {"articles": articles})
    if not result:
        print("  D1 unavailable — metadata saved in .all_articles.json only")
...
def call_groq_direct(prompt):
    """Call Groq API directly — most reliable, no gateway needed."""
    if not GROQ_API_KEY:
...
def generate_analysis(title, snippet, article_text, category):
    """Try Groq direct → Gemini direct → structured fallback."""
    prompt = build_prompt(title, snippet, article_text, category)

    # ── Primary: Groq direct ─────────────────────────────────────────────
    for attempt in range(MAX_RETRIES):
        try:
            text = call_groq_direct(prompt)
            if text:
                print(f"  ✓ Groq LLaMA 3.3 70B direct: {len(text)} chars")
                return text, True
        except HTTPError as e:
            err = str(e)
            ...
            if e.code == 429:
                ...
                print(f"  Groq HTTP {e.code}: {body[:120]}")
                break
        except Exception as e:
    ...
    print("  Trying Gemini direct fallback…")
    for attempt in range(MAX_RETRIES):
        try:
            text = call_gemini_direct(prompt)
            if text:
                print(f"  ✓ Gemini fallback: {len(text)} chars")
                return text, True
        except HTTPError as e:
            body = e.read().decode() if hasattr(e, 'read') else ""
            if e.code == 429:
                ...
                print(f"  Gemini HTTP {e.code}: {body[:120]}")
                break
        except Exception as e:
...
<head>
  ...
  <style>
    ...
    .art-hero {{
      ...
      border-bottom:1px solid #1e2535; padding:88px 24px 52px; text-align:center;
    }}
    .art-hero .back {{
      display:inline-flex; align-items:center; gap:6px; color:#00d4ff;
      text-decoration:none; font-size:.82rem; font-weight:600;
      letter-spacing:.06em; text-transform:uppercase; margin-bottom:28px; transition:opacity .2s;
    }}
    .art-hero .back:hover {{ opacity:.65; }}
    .art-hero .badge {{
      display:inline-block; padding:4px 13px; border-radius:4px;
      background:#00d4ff1a; color:#00d4ff; font-size:.7rem; font-weight:700;
      letter-spacing:.12em; text-transform:uppercase; border:1px solid #00d4ff40; margin-bottom:22px;
    }}
    .art-hero h1 {{
      font-size:clamp(1.45rem,4vw,2.35rem); font-weight:700; color:#f0f4ff;
      line-height:1.3; max-width:820px; margin:0 auto 18px; letter-spacing:-.015em;
    }}
    ...
    .art-body p:first-child::first-letter {{
      ...
      float:left; line-height:.72; margin:8px 12px 0 0; font-family:'Georgia',serif;
    }}
    .art-source {{
      max-width:760px; margin:0 auto 40px; padding:20px 24px 0;
      border-top:1px solid #1e2535; display:flex; align-items:flex-start; gap:10px; flex-wrap:wrap;
    }}
    ...
    .art-cta {{
      ...
      max-width:760px; margin:0 auto 80px; padding:38px 32px; text-align:center;
    }}
    ...
    .art-cta a  {{
      ...
      color:#fff; font-weight:600; text-decoration:none; font-size:.9rem; transition:opacity .2s;
    }}
...
def blog_index_html(all_articles):
    ...
    filter_btns = [
        ("all","All Articles"),("crypto","Crypto Law"),("fintech","Fintech"),
...
<head>
  ...
  <style>
    ...
    .blog-hero {{
      ...
      border-bottom:1px solid #1e2535; padding:100px 24px 60px; text-align:center;
    }}
    .blog-hero .eyebrow {{
    ...
    .blog-hero h1 {{
      font-size:clamp(1.9rem,5vw,3.1rem); font-weight:800; color:#f0f4ff;
      margin:0 0 14px; letter-spacing:-.022em; line-height:1.15;
    }}
    .blog-hero h1 span {{ color:#00d4ff; }}
    .blog-hero .sub {{ font-size:1rem; color:#6b7a99; max-width:540px; margin:0 auto; line-height:1.7; }}
    .blog-hero .pill {{
      display:inline-block; margin-top:20px; padding:6px 18px; border-radius:20px;
      background:#00d4ff12; border:1px solid #00d4ff30; color:#00d4ff; font-size:.78rem; font-weight:600;
    }}
    ...
    .fbtn {{
      ...
      background:transparent; color:#8a94b0; cursor:pointer; font-size:.82rem; font-weight:500; transition:all .2s;
    }}
    ...
    .card-badge {{
      ...
      color:#00d4ff; background:#00d4ff10; border:1px solid #00d4ff30;
      border-radius:4px; padding:3px 9px; display:inline-block; width:fit-content; margin-bottom:14px;
    }}
...
def main():
    print("=" * 60)
    print("  Web3Legals Crypto Radar — Phase 1 Hybrid")
    print("=" * 60)
    print(f"  Run time : {datetime.now(timezone.utc).isoformat()}")
    print(f"  AI route : Groq direct → Gemini direct")
    print(f"  DB       : Cloudflare D1 → local fallback")
    ...
    print(f"  Total: {len(candidates)} raw | ", end="")
    ...
    for i, art in enumerate(to_process):
        ...
        print(f"    Context: {'full text' if art_text else 'RSS snippet'}")
    ...
    # Merge new metadata into local backup (in case D1 is unavailable)
    existing_aids = {a["aid"] for a in all_arts}
    ...
    print(f"  Done — {len(all_arts)} articles in index")
